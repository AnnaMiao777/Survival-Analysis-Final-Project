...
par(mgp=c(2.5,1,0))
plot(nonpara.model,
     main="Kaplan Meier Survival Probability Estimates", ylab =c(expression(paste(hat(S),"(t)"))), xlab="Time",
     col=c("black","red","blue"))
legend("topright",
       legend=c("Obs", "Lev", "Lev+5FU"), col=c("black", "red", "blue"), lty=1,
       cex=0.8,
       lwd=2,
       bty="n")
...
#AFT MODEL
model.AFT <-survreg(Surv(time,status)~rx + sex + age + obstruct + perfor + adhere + nodes + differ + extent + surg + node4,
                    data = data,
                    dist = "weibull")
summary(model.AFT)
...
#Reduced AFT
model.AFT2 <-survreg(Surv(time,status)~ rx + obstruct + adhere + nodes + extent + surg + node4,
                     data = data,
                     dist = "weibull")
summary(model.AFT2)
